...
#define IR_FEEDBACK_MS 75           // time for IR feedback LED to stay on
#define IR_PAYLOAD_SIZE 21          // whirlpool AC
#define MANUAL_SYNC_DEBOUNCE_MS 300 // debounce button to ensure multiple syncs aren't fired
...
#define USE_HA_MQTT_DISCOVERY 1 // set 0 to disable Home Assistant auto-configure via MQTT discovery

// Home Assistant discovery payload
const char* HA_MQTT_DISCOVERY = ""\
    "{"\
        "\"name\":\"%s\","\
        "\"unique_id\":\"%s\","\
        "\"modes\":[\"off\",\"cool\"],"\
